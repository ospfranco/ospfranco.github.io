...
- We will use a crate called `cbindgen` that will help us generate a C header for our Rust functions. We will automate the header creation by creating a `build.rs` that runs everytime our project is compiled/checked. First we are going to add the dependency as a `[build-dependency]`, the project `Cargo.toml`:
